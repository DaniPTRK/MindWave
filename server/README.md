...
only), a sibling Flower FL aggregator, Prometheus metrics, and an append-only
audit trail. Deploys with one Docker Compose command.
...
- **No raw biometrics ever leave the device.** The server sees only account
  metadata, model weight tensors, and k-anonymized rollups (n ≥ 5).
...
4. **FL round** — after local fine-tuning the `FLTrainingWorker` uploads the
   round summary via Flower gRPC.
