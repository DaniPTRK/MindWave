...
def synthetic_bvp_60s() -> np.ndarray:
    """Synthetic BVP signal for 60s at 64 HZ, simulating waveform"""
    fs = WRIST_FS["BVP"]
    t = np.arange(WINDOW_SECONDS * fs) / fs
    signal = np.sin(2 * np.pi * 1.17 * t) + 0.3 * np.sin(2 * np.pi * 2.34 * t) # waveform-ish
    return signal.astype(np.float32)


@pytest.fixture
def synthetic_eda_60s() -> np.ndarray:
    """Synthetic EDA signal for 60s at 4 Hz with tonic + phasic components."""
    fs = WRIST_FS["EDA"]
    n = WINDOW_SECONDS * fs
...
def synthetic_temp_60s() -> np.ndarray:
    """Synthetic skin temperature for 60s at 4 Hz."""
    fs = WRIST_FS["TEMP"]
    n = WINDOW_SECONDS * fs
...
def synthetic_acc_60s() -> np.ndarray:
    """Synthetic ACC magnitude signal for 60s at 32 Hz."""
    fs = WRIST_FS["ACC"]
    n = WINDOW_SECONDS * fs
