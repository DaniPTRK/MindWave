...
class FeatureExtractorInstrumentedTest {
    ...
    private fun makeInterpreter(buf: java.nio.MappedByteBuffer): Interpreter {
        return Interpreter(buf.duplicate(), Interpreter.Options().apply { numThreads = 1 })
    }
    ...
    private fun getStressLogit(
        ...
        return try {
            // Must use the named "infer" signature — interp.run() invokes the
            // default (train) signature which requires optimizer state variables.
            interp.runSignature(
                mapOf("x" to inputBuf),
                mutableMapOf<String, Any>("logits" to outputBuf),
                "infer",
            ...
            outputBuf.rewind()
            outputBuf.float          // non_stress logit
            outputBuf.float          // stress logit — returned
        } catch (e: Exception) {
